...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "cropcircle", "tests")); system.time(test_check("cropcircle", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran cropcircle_$(VERSION).tar.gz
	rm -rf cropcircle.Rcheck/
